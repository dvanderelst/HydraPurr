...
def parser_hex_len26(body: bytes):
    ...
    pairs, rev_hex = [up[i:i+2] for i in range(0, 26, 2)], None
...
class TagReader:
    ...
    def tick_reset(self, now):
        ...
        if self.rst_state=="settling" and now>=self.rst_until_ms:
            self.rst_state="idle"; self.next_reset_ms+=self.period_ms; debug("[RFID] reset: settled"); return
    ...
    def poll(self):
        ...
        if (len(self.buf)-i)>self.max_len:
            ...
            del self.buf[:i+1]; return None
        j=self.buf.find(self.etx,i+1,min(len(self.buf),i+self.max_len))
        if j<0: return None
        frame=bytes(self.buf[i:j+1]); del self.buf[:j+1]
        body=self.validate_frame(frame);
        if body is None: return None
        pkt=self.parse_body(body)
        if pkt is None: return None
        pkt["raw_hex"]=body.hex().upper()
        if "tag_key" not in pkt: pkt["tag_key"]=pkt.get("tag") or pkt.get("id_hex") or pkt.get("id_ascii") or pkt["raw_hex"]
        pkt["time_ms"]=now
        key=pkt.get("tag_key")
