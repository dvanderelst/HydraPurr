...
def main_loop(clear_log=True, level=INFO):
    # Set up logging
            ...
            if tag:
                ...
                if (name is None) and not ALLOW_UNKNOWN:
                    ...
                    if active_tag is None:
                        ...
                        # write the screen when the cat gets activated - to avoid using to much screen update time
                        ctr = lickcounters.get(active_name)              # <— define ctr here
                        bout = ctr.bout_count if ctr else 0
                        hydrapurr.write_line(0, f"[A]{active_name}")
                        hydrapurr.write_line(1, f"[B]{bout}")
                    elif tag == active_tag:
                        ...
                        if quiet_ms >= LOCK_RELEASE_MS:
                            info(f"[Main Loop] Switch {active_name or 'unknown'} → {name or 'unknown'}")
                            active_tag = tag
                            active_name = name
                            last_seen_ms = t_ms
                        else:
                            # conflict within lock window; ignore the new tag
                            warn("[Main Loop] tag conflict ignored (within lock window)")
        ...
        if (active_tag is not None) and not gate_on:
            if TAG_REQUIRED and (active_name is not None):
                ...
                try:
                    ...
                    pass
            info(f"[Main Loop] Deactivating {active_name or 'unknown'}")
            # write the screen when the cat gets deactivated - to avoid using to much screen update time
            ctr = lickcounters.get(active_name)              # <— define ctr here
            bout = ctr.bout_count if ctr else 0
            hydrapurr.write_line(0, f"[D]{active_name}")
            hydrapurr.write_line(1, f"[B]{bout}")
            ...
            if active_name is not None and gate_on:
                ...
                if ctr:
                    ...
                    if previous_ctr_state != current_ctr_state:
                        info(f'[Main Loop] current counter state {current_ctr_state}')
                        previous_ctr_state = current_ctr_state
                        bout = ctr.bout_count if ctr else 0
                        # I might switch this off if it takes to much screen write time
                        if previous_bout_nr != bout:
                            hydrapurr.write_line(1, f"[B]{bout}")
                            previous_bout_nr = bout
            else:
